function Sigma = UpdateCMA(X,Sigma)
...
%--------------------------------------------------------------------------
    ...
    Sigma.cmaGen =  gen + 1;
    Converged = mean(sqrt(sum((X - mean(X)).^2, 2))) < 1e-7;
    if ConditionCov || NoEffectCoord || NoEffectAxis || TolXUp || Converged
        Sigma = struct('s',Sigma.s,'x',[],'sigma',0.5,'C',eye(n),'pc',0,'ps',0, 'bx', Sigma.bx, 'bf', Sigma.bf, 'valid', Sigma.valid, 'cmaGen', Sigma.cmaGen, 'ter', Sigma.ter);
    end
end